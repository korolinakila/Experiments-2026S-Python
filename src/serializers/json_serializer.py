...
class ExperimentSerializer:
    ...
    def __init__(self, experiment: "Experiment") -> None:
        """Инициализировать сериализатор с экземпляром эксперимента."""
        self._experiment = experiment

    def _clear_experiment(self) -> None:
        """Очистить текущее состояние эксперимента перед загрузкой из файла."""
        self._experiment._variables.clear()
        self._experiment._constants.clear()
        self._experiment._instruments.clear()

    def save(self, filepath: Path, data_dir: Path) -> None:
        """Сохранить эксперимент в JSON + CSV."""
        from .csv_handler import CSVHandler

        data = {"experiment": {"variables": [], "constants": [], "instruments": []}}
